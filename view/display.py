...
import os
from project.Display_wrapper import add_title
from project.Model import DataRetrieverAdapter

#Using Decorator Pattern from  Display_wrapper to add title 
@add_title("Wall Street Bulls (WSB)")
def generate_combined_html(fngd_file, fngu_file):
    
    # Read the JSON files into pandas DataFrames
    price_data = DataRetrieverAdapter.load_price_data()

    fngd = price_data['FNGD']
    fngu = price_data['FNGU']
